//! Frost-ed25519 participant-side DKG (Distributed Key Generation).
...
/// Participant-side FROST(ed25519) key generation protocol instance.
pub struct FrostEd25519NodeKeyGeneration {
...
impl FrostEd25519NodeKeyGeneration {
...
#[async_trait]
impl Protocol for FrostEd25519NodeKeyGeneration {
            ...
            (1, 1, FrostWire::DkgRound1Packages { packages }) => {
                let secret: round1::SecretPackage =
                    self.round1_secret.take().ok_or_else(|| {
                        Error::InvalidState("Missing round 1 secret.".into())
                    })?;
            ...
            (2, 2, FrostWire::DkgRound2Packages { packages }) => {
                let secret: &round2::SecretPackage =
                    self.round2_secret.as_ref().ok_or_else(|| {
                        Error::InvalidState("Missing round 2 secret.".into())
                    })?;
    ...
    async fn finalize(&mut self) -> Result<ProtocolOutput, Error> {
        let key_package: KeyPackage =
            self.key_package.take().ok_or_else(|| {
                Error::InvalidState("Missing key package.".into())
            })?;

        let public_key_package: PublicKeyPackage =
            self.public_key_package.take().ok_or_else(|| {
                Error::InvalidState("Missing public key package.".into())
            })?;
